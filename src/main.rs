...
use poise::{
    ...
    serenity_prelude::{
        ClientBuilder, Color, CreateAttachment, CreateEmbed, CreateEmbedAuthor, CreateEmbedFooter,
        GatewayIntents,
    },
};
...
async fn cat(ctx: Context<'_>) -> Result<(), anyhow::Error> {
    // fetch a cat :3
...
/// alfred delfin
#[poise::command(slash_command, prefix_command, track_edits)]
async fn delfin(ctx: Context<'_>) -> Result<(), anyhow::Error> {
...
/// alfred eminem
#[poise::command(slash_command, prefix_command, track_edits)]
async fn eminem(ctx: Context<'_>) -> Result<(), anyhow::Error> {
...
/// alfred kleanthis
#[poise::command(slash_command, prefix_command, track_edits)]
async fn kleanthis(ctx: Context<'_>) -> Result<(), anyhow::Error> {
